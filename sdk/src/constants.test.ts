	...
	describe("DynamoDB", () => {
		...
		describe("Table.ARN", () => {
			it("should generate full ARN with custom config", () => {
				...
				expect(arn).toBe(
					"arn:aws:dynamodb:us-west-2:123456789012:table/PROD-v1-Audit",
				);
			});
			...
			it("should use process.env values when no config provided", () => {
				...
				const arn = DynamoDB.Table.ARN();
				expect(arn).toBe(
					"arn:aws:dynamodb:eu-west-1:987654321098:table/DEV-v1-Audit",
				);
	...
	describe("EventBridge", () => {
		...
		describe("DetailType", () => {
			...
			it("should satisfy AnyEventBridgeDetailType", () => {
				const upserted: AnyEventBridgeDetailType =
					EventBridge.DetailType.UPSERTED;
				const deleted: AnyEventBridgeDetailType =
					EventBridge.DetailType.DELETED;
		...
		describe("Bus.ARN", () => {
			it("should generate full ARN with custom config", () => {
				...
				expect(arn).toBe(
					"arn:aws:events:us-west-2:123456789012:event-bus/PROD-v1-Audit",
				);
			});
			...
			it("should use process.env values when no config provided", () => {
				...
				const arn = EventBridge.Bus.ARN();
				expect(arn).toBe(
					"arn:aws:events:eu-west-1:987654321098:event-bus/DEV-v1-Audit",
				);
			...
			it("should default region to us-east-1 when AWS_REGION is undefined", () => {
				...
				const arn = EventBridge.Bus.ARN();
				expect(arn).toBe(
					"arn:aws:events:us-east-1:111222333444:event-bus/QA-v1-Audit",
				);
